...
void arm7_fifo() { // check incoming fifo messages
   while(!(REG_IPC_FIFO_CR & IPC_FIFO_RECV_EMPTY)) // -aj
     {
...
//---------------------------------------------------------------------------------
	...
	IPC->soundData = 0;
	*(vu32 *)0x27FFFFC = 0; // arm7 loop address to default -aj
	
	irqInit();
	irqSet(IRQ_VBLANK, VblankHandler);
	irqEnable(IRQ_VBLANK);
	
	irqSet(IRQ_WIFI, Wifi_Interrupt); // set up wifi interrupt -uncomm aj
	irqEnable(IRQ_WIFI);
